...
int main(){
    ...
    for(;;){
        ...
        for(unsigned int i=0; i<64; i++){
            auto line = hwlib::line(hwlib::xy(0,0), hwlib::xy(5,5), hwlib::color(255-(i*4), (i*4), 0));
            line.draw(w);
            w.flush();
        }for(unsigned int i=0; i<64; i++){
            auto line = hwlib::line(hwlib::xy(0,0), hwlib::xy(5,5), hwlib::color(0, 255-(i*4), (i*4)));
            line.draw(w);
            w.flush();
        }for(unsigned int i=0; i<64; i++){
            auto line = hwlib::line(hwlib::xy(0,0), hwlib::xy(5,5), hwlib::color((i*4), 0, 255-(i*4)));
            line.draw(w);
            w.flush();
        }
    }
